...
import { updateCustomer } from "../../api/employeeApi";
import { getAllEmployees } from "../../api/adminApi";
import { AuthContext } from "../../context/AuthContext";
import { toast } from "react-hot-toast";

export const EditCustomerForm = ({ customer, onSuccess, onCancel }) => {
  const { user } = useContext(AuthContext);
  const [formData, setFormData] = useState({
    name: customer?.name || "",
    email: customer?.email || "",
    phone: customer?.phone || "",
    assignedToUserId: customer?.assignedToId || ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare data - only send assignedToUserId if it's admin and it changed or is set
    const submitData = {
      name: formData.name,
    ...
    if (isAdmin && formData.assignedToUserId) {
